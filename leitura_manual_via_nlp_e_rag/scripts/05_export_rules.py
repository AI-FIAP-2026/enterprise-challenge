...
INPUT_PATH = Path("outputs/regras_estruturadas.json") #---> Caminho do arquivo JSON com  as regras estruturadas extraídas e validadas pelo modelo, que podemos exportar tanto para CSV quanto Excel.
...
if __name__ == "__main__": #---> O bloco principal do código é executado quando o script é rodado diretamente.
    ...
    df = pd.DataFrame(rules) #---> Criamos um DataFrame do pandas a partir da lista de regras estruturadas, para facilitar a manipulação/exportação dos dados
    ...
    print(f"Excel salvo em: {XLSX_PATH}") #---> E exibimos o caminho onde o arquivo Excel foi salvo