...
def rodar_treinamento_queimadas(progress_callback=None):
    ...
    atualizar_progresso("Extração", 30, "Lendo query SQL de queimadas (2025) em lotes...")
    
    caminho_sql = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'sql', 'ml_queimadas.sql'))
    ...
    try:
        ...
        print(f"🔴 Erro ao ler o arquivo SQL de queimadas em {caminho_sql}: {e}")
        connection.close()
        return None
    ...
    atualizar_progresso("Pré-processamento", 50, "Calculando features temporais no treino...")
    for col in df.columns:
        if col not in ['uf', 'regiao', 'municipio', 'cobrade', 'grupo_desastre', 'codigo_ibge', 'data_hora_br', 'inc_data_evento']:
            df[col] = pd.to_numeric(df[col], errors='coerce')
    
    df = df.dropna(subset=['temperatura', 'umidade', 'velocidade_vento', 'precipitacao'])
    
    # Engenharia de features temporal idêntica ao serviço de inferência
    if 'data_hora_br' in df.columns and 'codigo_ibge' in df.columns:
        ...
        df['chuva_acumulada_15d'] = df.groupby('codigo_ibge')['precipitacao'].transform(lambda x: x.rolling(15, min_periods=1).sum())
    else:
        df['temp_media_7d'] = df['temperatura']
        df['umidade_media_7d'] = df['umidade']
        df['chuva_acumulada_15d'] = df['precipitacao']
    ...
    print(f"🎯 Distribuição da classe alvo ({coluna_target}):\n{df[coluna_target].value_counts().to_dict()}")

    limite_seguranca = 75000
    ...
    print(f"📊 Total de registros efetivos para treino de queimadas: {total_registros:,}")
    ...
    atualizar_progresso("Treinamento", 75, "Treinando Random Forest com as 7 features...")
    ...
    # 💾 SALVANDO O MODELO COM AS 7 FEATURE NAMES CORRETAS
    caminho_modelo_pkl = os.path.abspath(os.path.join(os.path.dirname(__file__), 'ml_queimadas_treinado.pkl'))
    joblib.dump(modelo, caminho_modelo_pkl)
    print(f"💾 [SUCESSO] Modelo salvo em: {caminho_modelo_pkl}")

    atualizar_progresso("Avaliação", 90, "Avaliando desempenho com limiar otimizado...")
    
    y_prob = modelo.predict_proba(X_test)[:, 1]
    limiar_alerta = 0.15
    y_pred = (y_prob >= limiar_alerta).astype(int)
...
if __name__ == "__main__":
    print("==========================================================")
    print(" 🔥 INICIANDO TREINAMENTO: MODELO DE QUEIMADAS (ATUALIZADO)")
    print("==========================================================")
    
    resultados = rodar_treinamento_queimadas()
    
    if resultados:
        report = resultados['report_dict']
        print("\n" + "="*60)
        print(" 📊 RESULTADOS DO MODELO DE QUEIMADAS")
        print("="*60)
        ...
        print(f" Recall (Captura de Fogo)..: {report.get('1', {}).get('recall', 0):.4f}")
        print(f" F1-Score (Queimadas)......: {report.get('1', {}).get('f1-score', 0):.4f}")
        
        print("\n" + "-"*60)
        print(" ⚖️ ANÁLISE COMPARATIVA: RISCO DE SEGUROS VS. MODELO")
        print("-" * 60)
        
        tabela_comparativa = pd.DataFrame({
            "Métrica de Avaliação": [
                "AUC-ROC Score", 
                "Recall (Sensitividade - Foco em Pegar Fogo)", 
        ...
        print("⚠️ O treinamento não retornou resultados válidos.")